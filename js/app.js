...
function setupLinePage() {
    // 自動/手動切り替えボタン
    const autoToggleBtn = document.getElementById('line-auto-mode-toggle');
    if (autoToggleBtn) {
        autoToggleBtn.addEventListener('click', toggleLineAutoMode);
    }
}

function renderLineConversations() {
    const conversations = loadLineConversations();
    const listContainer = document.getElementById('line-conversations-list');
    
    if (!listContainer) {
        console.warn('LINE会話リストコンテナが見つかりません');
        return;
    }
    ...
    if (conversations.length === 0) {
        listContainer.innerHTML = `
            <div class="empty-state" style="text-align:center; padding:40px; color:#6C757D;">
                <p>💬 LINE会話がまだありません</p>
                <p style="font-size:14px; margin-top:8px;">お客様からメッセージが届くとここに表示されます</p>
            </div>
        `;
        return;
    }
    ...
    listContainer.innerHTML = conversations.map(conv => {
        ...
        return `
            <div class="line-conversation-card" onclick="openLineChat('${conv.userId}')">
                <div class="conversation-header">
                    <div class="user-info">
                        <div class="user-avatar">${conv.displayName.charAt(0)}</div>
                        <div>
                            <h4>${conv.displayName} ${unreadBadge}</h4>
                            <p class="timestamp">${formatTimestamp(conv.lastMessageTime)}</p>
                        </div>
                    </div>
                </div>
                <div class="last-message">${conv.lastMessage || 'メッセージがありません'}</div>
            </div>
        `;
    }).join('');
}

function toggleLineAutoMode() {
    const currentMode = localStorage.getItem('line_auto_mode') === 'true';
    const newMode = !currentMode;
    
    localStorage.setItem('line_auto_mode', newMode);
    
    const btn = document.getElementById('line-auto-mode-toggle');
    if (btn) {
        if (newMode) {
            btn.textContent = '🤖 自動モード ON';
            btn.classList.remove('btn-secondary');
            btn.classList.add('btn-success');
        } else {
            btn.textContent = '✋ 手動モード';
            btn.classList.remove('btn-success');
            btn.classList.add('btn-secondary');
        }
    }
    
    alert(newMode ? '自動返信モードをONにしました' : '手動返信モードに切り替えました');
}

function openLineChat(userId) {
    console.log('LINE チャット開く:', userId);
    alert(`LINE チャット画面（ユーザーID: ${userId}）\n※ 実装予定`);
}
...
window.openLineChat = openLineChat;
...
function renderCustomersTable() {
    ...
    if (customers.length === 0) {
        customersTable.innerHTML = `
            <div class="empty-state" style="text-align:center; padding:40px; color:#6C757D;">
                <p>👤 顧客データがまだありません</p>
                <p style="font-size:14px; margin-top:8px;">予約データから自動的に顧客情報が作成されます</p>
            </div>
        `;
        return;
    }
            ...
            <tbody>
                ${customers.map(c => `
                    <tr>
                        ...
                        <td>${c.lastVisit}</td>
                        <td>
                            <button class="btn btn-primary" onclick="viewCustomerDetail('${c.id}')">詳細</button>
                        </td>
                    </tr>
                `).join('')}
            </tbody>
...
window.renderLineConversations = renderLineConversations;
window.renderCustomersTable = renderCustomersTable;

console.log('✅ app.js v3.0 完全版 読み込み完了');
